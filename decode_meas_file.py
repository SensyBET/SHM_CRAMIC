...
if 0:
    ...
    FOLDER_MEASUREMENT       = "data_sensy\\CABIN2\\data_2022_08_22_144045"
    FILENAME_MONITOR_CONFIG  = "config\\default\\monitor_user_settings.xml"
    FILENAME_MONITOR_SCALING = "config\\default\\monitor_scaling.txt"
...
def create_new_output_file ():
    ...
    global header
    # Create output file
    output_file_name = FOLDER_MEASUREMENT + "_" + str(file_idx) + ".csv"
...
folder_path = os.getcwd() + '\\' + FOLDER_MEASUREMENT
folder_path = os.path.join(script_dir,FOLDER_MEASUREMENT)
print("folder_path=", folder_path)
in_files = [f for f in os.listdir(folder_path) if f.lower().endswith('.bin')]
in_files.sort()
debug_print(1, in_files)

# for log purpose
files_nb = len(in_files)
files_cnt = 1

new_file = create_new_output_file()
line_cnt = 0

# open each file and append data in output file
for fname in in_files:
    ...
    with open(fpath, 'rb') as file:
        print("{:03d}/{} {}".format(files_cnt, files_nb, fname))
        eof_reached = False
        data = list()
        TimeIdx = 1 #[1;250]
        phase = PH_0_INIT
